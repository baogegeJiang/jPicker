...
for i=1:length(quake)
    ...
    if quake(i).CC<0.25;continue;end
    ...
    title(sprintf('quake %d %s  la %.1f lo %.1f dep:%.1f ml: %.2f tmpIndex: %d cc: %.3f',i,datestr(oTime),loc(2,i),loc(3,i),quake(i).PS(4),quake(i).PS(5),quake(i).tmpIndex,quake(i).CC),...
        'fontsize',7);
