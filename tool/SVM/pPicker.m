...
try
    jL=[-150:150];jL1=[-150:150];jLS1=[-250:250];
    isStrict=0;
    setPicker;
    jLS=jLS1;
    delta=0.02;
    ...
    if toDoLst(1)>0
        ...
        minIsPhase=0;
        [~,jLst]=getdetec(isPhase,minIsPhase,minD);
        if jLst(1)>0
    ...
    if toDoLst(2)==1
        ...
        if pre(pIndex)-floor(pre(pIndex))==0.1234567
            ...
            if pIndex-be1>0 && pIndex+af1<length(sta.data)
                [~,arDataT]=ArAic2(sta.data(pIndex-be1:pIndex+af1,3),arN,be2,af2);
                [~,pIndexTemp]=max(arDataT(be2:af2));
    ...
    if sA<0.3*pA && (sIndex-pIndex)/50>15;sTime=0;return;end
