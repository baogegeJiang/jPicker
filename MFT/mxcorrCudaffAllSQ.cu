...
void mexFunction(int nlhs,mxArray *plhs[],int nrhs,const mxArray *prhs[])
{
    ...
    float *addMatTmp=(float *)malloc(sizeof(float)*la0);
    ...
    for(int i=0;i<laout;i++){
       addMat[i]=0;}
    for(int i=0;i<N;i++){
       for(int j=laout-(int)bIndex[i]-lc0*2-lb0*2;i*laout+j>0&&j<laout;j++){
          c0[i*laout+j]=0;
       }
    }
