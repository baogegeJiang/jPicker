function[quake]=detQuake(sta,modPhase,modP,doAgain)
...
dLa=(R(2)-R(1))/nR;dLo=(R(4)-R(3))/mR;
%La=R(1):dLa:R(2);Lo=R(3):dLo:R(4);

%% find every stations' possible phase pairs and  stack them.
for i =1:length(sta)
    if sta(i).isF==0;continue;end
    fprintf('%d ',i);
    sta(i).pair=pairPick(sta(i).peak,sta(i).xd,sta(i).A,modP,sta(i).data,sta(i).bNum,delta,sta(i).pre);
...
[~,quakeTime]=getdetec(secCSMax,minDet,minD);
...
for i=1:length(quakeTime)
    ...
    for j=max(1,tempTime-4):length(secC)
        ...
        tempRMS=0;
        tempTimeO=j+0.5;
        [~,tmpIM]=find(secCS(j,1,:,:)==secCSMax(j));
...
qCount=0;
for i=1:length(quake)
    if sum(quake(i).pTime>0)<3;continue;end
    if loc(1,i)==0
        continue;
    ...
    for j=1:length(quake(i).pTime)
        ...
        %psL=max(1,pIndex-250):min(length(sta(j).data),1.5*sIndex-0.5*pIndex);
        %sta(j).data(psL,:)=sta(j).data(psL,:).*0;
        psL=max(1,pIndex-500):min(length(sta(j).pre),max(sIndex+750,1.5*sIndex-0.5*pIndex));
        try
...
if doAgain==0 && ~isempty(quake); if qCount~=0;quake=quake0;end;return;end
if qCount==0 || isempty(quake);quake=[];return;end
